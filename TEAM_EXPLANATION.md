...
실제 서비스로 발전하면 하루 전, 당일 아침, 충전 한 시간 전에 예보를 다시 받아 아직 충전하지 않은 시간만 재계산해야 합니다. 현재 MVP는 Open-Meteo의 제주시 내일 예보를 30분마다 다시 받을 수 있지만, 충전 도중 자동 재계획은 아직 구현하지 않았습니다.
...
```text
실제 Green Score가 70점 이상인 충전량 × 20P/kWh
```
...
한 번 충전당 총 포인트는 1,500P를 넘지 않습니다. `1P=1원`은 체감효과 비교를 위한 가정일 뿐이며, 이 숫자를 실제 지급액이라고 말하면 안 됩니다. `정책 단가를 가정한 과거 재현 결과`라고 설명합니다.
...
| `tests/test_optimizer.py` | 포인트와 최적화의 핵심 규칙 7개 검사 |
...
6. `내일 예보 실험`으로 바꾸어 실제 API 기반 24시간 화면을 보여줍니다.
7. 이 모드의 보너스는 충전 후 실제값이 들어와야 정산된다고 설명합니다.
8. 출발시간을 너무 이르게 바꿔 `목표 충전 불가능` 경고를 보여줍니다.
9. 마지막으로 구현·미구현 항목을 공개합니다.
