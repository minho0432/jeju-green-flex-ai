...
실제 서비스로 발전하면 하루 전, 당일 아침, 충전 한 시간 전에 예보를 다시 받아 아직 충전하지 않은 시간만 재계산해야 합니다. 현재 MVP는 Open-Meteo의 제주시 오늘·내일 예보를 30분마다 다시 받을 수 있습니다. Open-Meteo는 실시간 측정기가 아니라 기상모델의 예보이므로 갱신 직후에도 비구름·바람을 틀릴 수 있습니다.

그래서 추가로 여러 날씨 시나리오를 제공하는 Open-Meteo Ensemble API를 붙였습니다. 각 날씨 시나리오를 AI 모델에 다시 넣어 재생에너지와 SMP의 가능한 범위를 만들고, 기존 과거오차 범위에 추가합니다. 날씨 시나리오 차이가 크면 보수적 점수가 더 낮아져 너무 낙관적인 시간을 피할 수 있습니다.
...
| `scripts/weather_ensemble.py` | 여러 날씨 시나리오를 AI에 통과시켜 불확실성 확대 |
| `scripts/jeju_grid_live.py` | KPX 제주 5분 실측을 읽고 시간별 MWh로 변환 |
| `scripts/ev_charger_live.py` | 제주 충전소 조회, 안전한 XML 파싱, 추천시간 적합성 검사 |
...
| `tests/test_weather_ensemble.py` | 날씨 시나리오와 예상범위 확대 4개 검사 |
| `tests/test_ev_charger_live.py` | 제주 충전소 위치·상태·출력·운영시간·오류 10개 검사 |
...
9. `내일 예보 실험`으로 바꾸어 실제 기상예보 API 기반 24시간 화면과 날씨 시나리오 수를 보여줍니다.
10. 충전소 API 활용승인이 있다면 위치·상태를 불러와 현재 충전대기·계획 출력·추천시간 운영 여부가 모두 맞는 충전기를 걸러 보여줍니다.
11. 미래 모드의 보너스는 충전 후 실제값이 들어와야 정산된다고 설명합니다.
12. 출발시간을 너무 이르게 바꿔 `목표 충전 불가능` 경고를 보여줍니다.
13. 마지막으로 구현·미구현 항목을 공개합니다.

## 14. 30초 발표 대본

> 제주 개인 전기차 사용자는 언제 충전해야 재생에너지 활용에 유리한지 알기 어렵고, 기다려서 충전할 직접적인 혜택도 없습니다. 저희는 AI로 재생에너지 발전량과 SMP 보조지표를 예측해 출발 전 목표 배터리를 지키는 연속 충전시간을 추천합니다. 시간순 5회 검증에서 강한 단순 기준보다 SMP 오차를 4.22%, 재생에너지 오차를 30.66% 줄였습니다. KPX 공식 5분 태양광·풍력 실측이 도착하면 남은 예측과 충전계획을 다시 계산하고, 추천 참여에 대한 Green Point 정책도 시뮬레이션합니다.
...
현재는 과거 오차로 만든 범위에 여러 날씨 시나리오의 AI 결과를 추가하고, 불리한 값을 사용해 추천합니다. 내일 예보 화면도 30분마다 새로 받을 수 있습니다. 실제 서비스에서는 충전 중에도 예보를 갱신하고 미래의 미충전 구간만 다시 계산해야 합니다. 보장 포인트는 유지하고 성과 보너스만 실제 결과로 정산합니다.
...
> 현재 데모에는 2025년 시간별 데이터로 검증한 AI, 목표 배터리와 출발시간을 지키는 충전시간 계산, 예산 기반 Green Point, Open-Meteo 일반예보와 여러 날씨 시나리오, KPX 제주 5분 태양광·풍력 실측, 한국환경공단 제주 충전소 선택 조회까지 들어갔습니다. 공식 실측은 12개가 모두 모인 시간만 AI 보정에 쓰고 자료 지연 여부도 표시합니다. 아직 실제 충전기 예약, 충전 세션 확인, 결제와 포인트 지급은 구현하지 않았습니다. 따라서 이번 해커톤에서는 `AI 추천과 정책 시뮬레이션`을 증명하고 실제 지급은 제휴가 필요한 다음 단계로 공개하겠습니다.
...
다음 개발 우선순위는 `충전소 API 활용승인·실응답 확인 → API 장애용 정적 위치 대체 경로 → 추천시간 알림 → 제휴 세션·결제 연동 → 실제 포인트 원장`입니다. 상세 API와 막히는 이유는 `API_ROADMAP.md`를 봅니다.
