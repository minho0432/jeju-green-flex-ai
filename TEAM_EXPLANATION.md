...
| `scripts/ev_charger_live.py` | 제주 충전소 위치·상태 조회와 안전한 XML 파싱 |
...
| `tests/test_ev_charger_live.py` | 제주 충전소 위치·상태·오류 4개 검사 |
...
10. 충전소 API 활용승인이 있다면 사이드바에서 위치·상태를 불러와 검색과 지도를 보여줍니다.
...
다음 개발 우선순위는 `충전소 API 활용승인·실응답 확인 → 추천시간 알림 → 제휴 세션·결제 연동 → 실제 포인트 원장`입니다. 상세 API와 막히는 이유는 `API_ROADMAP.md`를 봅니다.
