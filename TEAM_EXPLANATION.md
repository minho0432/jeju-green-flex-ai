...
이 문서는 코드를 모르는 사람도 프로젝트의 논리, AI, 리워드, 한계를 설명할 수 있도록 만든 발표자용 교과서입니다.

## 0. 먼저 외울 한 문장

> 사용자가 현재 배터리와 출발시간을 입력하면, AI가 제주 SMP와 재생에너지 발전량을 예측하고 예측 오차까지 고려해 충전하기 좋은 연속 시간대를 추천합니다. 리워드는 추천 참여 보장분과 실제 성과 확인분으로 나눠 계산합니다.
...
실제 서비스로 발전하면 하루 전, 당일 아침, 충전 한 시간 전에 예보를 다시 받아 아직 충전하지 않은 시간만 재계산해야 합니다. 현재 MVP에는 실시간 재예보 API가 연결되어 있지 않습니다.
...
- 가격 기회점수: 과거 2025년과 비교해 SMP가 낮을수록 높음
- 재생에너지 기회점수: 과거 2025년과 비교해 발전량이 많을수록 높음

재생에너지 비중을 55%로 조금 더 크게 둔 것은 서비스 목표가 친환경 시간으로 충전을 이동시키는 것이기 때문입니다. 이 가중치는 법이나 과학의 정답이 아니라 MVP의 운영정책입니다. 발표에서 이 점을 숨기지 않습니다.
...
따라서 리워드는 자동으로 생기는 돈이 아닙니다. 다음 중 한 곳이 캠페인 예산을 제공해야 합니다.
...
```text
실제 Green Score가 기준 이상인 충전량 × 20원/kWh
```
...
이 숫자를 실제 지급액이라고 말하면 안 됩니다. `정책 단가를 가정한 과거 재현 결과`라고 설명합니다.

## 11. 실제 페이백에 추가로 필요한 것
...
| `tests/test_optimizer.py` | 리워드와 최적화의 핵심 규칙 6개 검사 |
...
1. 기본값 SOC 30%→80%, 출발 20시를 보여줍니다.
2. 추천시간이 연속 구간으로 나오는 것을 보여줍니다.
3. `예측 오차까지 고려` 체크가 켜져 있음을 말합니다.
4. 예측값과 실제값 그래프를 보여줍니다.
5. 참여 보장분과 실제 성과 보너스가 분리된 것을 보여줍니다.
6. 출발시간을 너무 이르게 바꿔 `목표 충전 불가능` 경고를 보여줍니다.
7. AI 성능 검증 항목에서 5회 시간순 검증과 강한 기준 비교를 보여줍니다.
8. 마지막으로 구현·미구현 항목을 공개합니다.

## 14. 30초 발표 대본

> 전기차 사용자는 언제 충전해야 제주 재생에너지 활용에 유리한지 알기 어렵습니다. 저희는 2025년 제주 시간별 데이터를 이용해 SMP와 태양광·풍력 발전량을 예측하고, 사용자의 현재 배터리와 출발시간을 반드시 지키면서 가장 좋은 연속 충전시간을 추천합니다. 시간순 5회 검증에서 강한 단순 기준보다 SMP 오차를 4.22%, 재생에너지 오차를 30.66% 줄였습니다. 또한 예보 오류를 고려해 보수적인 점수를 사용하고, 리워드를 참여 보장분과 실제 성과 보너스로 분리했습니다. 현재는 과거 재현 시뮬레이션이며 향후 충전사업자와 연동해 실제 정산으로 확장할 수 있습니다.

## 15. 예상 질문과 답변

### Q. SMP가 낮으면 소비자가 정말 싸게 충전합니까?

아닙니다. SMP는 도매가격 지표이고 소비자 충전단가와 직접 같지 않습니다. 현재 리워드는 제휴사가 캠페인 예산을 제공한다고 가정한 시뮬레이션입니다.
...
현재는 과거 오차로 만든 예상범위의 불리한 값을 사용해 추천합니다. 실제 서비스에서는 예보를 여러 번 갱신하고 미래의 미충전 구간만 다시 계산해야 합니다. 보장 리워드는 유지하고 성과 보너스만 실제 결과로 정산합니다.
